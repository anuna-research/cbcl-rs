#!/usr/bin/env bash
# DeepSeek V3.2 N=100 sweep, 2026-05-07.
#
...
# processes. Routes through OpenRouter at deepseek/deepseek-v3.2.
#
...
DATE_TAG="2026-05-07"
ARCHIVE="crates/cbcl-arena/measurements/N100-deepseek-$DATE_TAG"
TRANSCRIPTS="crates/cbcl-arena/transcripts"

mkdir -p "$ARCHIVE"
MASTER_LOG="$ARCHIVE/master.log"
{
  echo "DeepSeek V3.2 N=100 sweep"
  echo "Date:       $DATE_TAG"
...
removed=$(find "$TRANSCRIPTS" -maxdepth 1 -name 'dsk32-*.jsonl' | wc -l | tr -d ' ')
find "$TRANSCRIPTS" -maxdepth 1 -name 'dsk32-*.jsonl' -delete
echo "  removed $removed dsk32-*.jsonl files (smoke contamination)" | tee -a "$MASTER_LOG"
