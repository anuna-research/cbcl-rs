...
| `role.rs`, `projection.rs` | `R6DCFLPreservation.lean` | DCFL preservation at R6, both halves: the role-annotation syntax (`(:roles …)`, `(* name)`, `:from`/`:to`, `(with-roles …)`) inhabits the existing S-expression DCFL grammar; and the causal trace language of a role-annotated protocol is regular — recognised by an explicit finite automaton — hence trace-DCFL, with projection reusing the identical recogniser builder (`dcfl_preserved_under_roles`, `causalTrace_regular`, `causalTrace_isTraceDCFL`, `projection_adds_no_recogniser`) |
...
- **`causalTrace_regular`** / **`projection_adds_no_recogniser`**: the paper's DCFL-preservation proposition, mechanised. The causal trace language of a role-annotated protocol is recognised exactly by a finite automaton (states: the sublists of the declared performative names plus a dead state — the verifier's unbounded consumed-name list provably collapses onto it), regular ⊆ DCFL is formal (the automaton wrapped as a stack-untouched DPDA), and a projected protocol's recogniser is *definitionally* the same builder applied to the projected steps — role-local verification needs no machine class beyond what the global protocol already has.
