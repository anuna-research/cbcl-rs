...
| `role.rs`, `r6.rs`, `projection.rs` | `EPP.lean`, `EPPCompletion.lean` | Endpoint-projection correspondence (run-level): `project`/`glue` are mutually inverse on closed configurations, and role-local verification is sound and complete for whole-conversation safety (`epp_correspondence`, `epp_correspondence_complete`) |
...
- **`openings_suffice`**: a typed field-opening (Merkle-over-fields, `typed_addr.rs`) carries exactly the predecessor type the verdict reads, so selective disclosure is sufficient for role-local safety without revealing the rest of the message.
